...
def process_grades(input_file, output_file):
    with open(input_file, mode='r', newline='') as infile, open(output_file, mode='w', newline='') as outfile:
        ...
        for row in reader:
            sid = row['SID']
            autograder_grade = next((value for key, value in row.items() if '1: Autograder' in key), None)
            dry_part_grade = next((value for key, value in row.items() if '2: Dry Part' in key), None)
